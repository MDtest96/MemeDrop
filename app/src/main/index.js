  ...
  if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    if (payload.showLocalPreview !== false) {
      const localDrop = {
        ...
        from: { id: "me", username: store.get("linkIdentity")?.username || "Moi" },
        ts: Date.now(),
      };
...
ipcMain.handle("drop:sendUrl", async (_e, payload) => {
  if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    // Local playback — skip if disabled
    if (payload.showLocalPreview !== false) {
      const localDrop = {
        type: "drop",
        media: {
...
ipcMain.handle("drop:preview", async (_e, payload) => {
  if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    // Local preview on overlay
    if (!overlayWin || overlayWin.isDestroyed()) createOverlayWindow();
    startTopGuard();
    enforceTop();
    const mediaUrl = `file:///${previewPayload.filePath.replace(/\\/g, "/")}`;
    overlayWin.webContents.send("drop", {
      type: "drop",
      media: { url: mediaUrl, kind: previewPayload.kind, mime: "image/jpeg" },
      caption: previewPayload.caption || null,
      rain: previewPayload.rain || null,
      from: { id: "me", username: store.get("linkIdentity")?.username || "Moi" },
      ts: Date.now(),
      settings: {
        volume: store.get("volume"),
        musicVolume: store.get("musicVolume"),
        duration: store.get("duration") || 4,
        videoDuration: store.get("videoDuration") || 30,
      },
    });
...
ipcMain.handle("tools:copyCommand", async (_e, { target, fileName, caption, rain }) => {
  const { clipboard } = require("electron");
  let cmd = `/memedrop ${fileName}`;
  if (target) cmd += ` --target "${target}"`;
  if (caption) cmd += ` --caption "${caption}"`;
  if (rain) cmd += ` --rain "${rain}"`;
