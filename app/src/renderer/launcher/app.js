...
// Fix Giphy scroll: ensure the grid can scroll vertically
if (giphyGrid) {
  giphyGrid.style.overflowY = "auto";
  giphyGrid.style.maxHeight = "60vh";
}
...
const unsubConn = window.memedrop.onConnection((state) => {
  ...
  // Update users count from connection state
  const usersCountEl = document.getElementById("users-count");
  if (usersCountEl && state.links) {
    const count = state.links.length || 0;
    usersCountEl.textContent = `${count || 0} connectés`;
  }
...
async function openDropPanel(meme) {
  ...
  // Re-add custom targets that were persisted in localStorage
  if (customTargets && customTargets.size > 0) {
    const existingValues = Array.from(panelTarget.options).map(o => o.value);
    for (const ct of customTargets) {
      if (!existingValues.includes(ct)) {
        const opt = document.createElement("option");
        opt.value = ct;
        opt.textContent = ct;
...
document.getElementById("btn-send")?.addEventListener("click", async () => {
  ...
  let lastTarget = "";
  const localPreview =
    document.getElementById("panel-local-preview")?.checked ?? false;
...
async function loadGiphy(query, reset = true) {
  ...
  try {
    ...
    if (reset && items.length === 0) {
      giphyGrid.innerHTML =
        'data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'100\' height=\'100\'><text y=\'50\' x=\'50\' text-anchor=\'middle\' font-size=\'12\'>Aucun résultat</text></svg>';
      }
    } catch (e) {
      console.error("loadGiphy error:", e);
      if (reset)
        giphyGrid.innerHTML =
          '<p style="grid-column:1/-1;text-align:center;color:var(--text-dim);">Erreur: ' + (e.message || 'inconnue') + '</p>';
  } finally {
...
function createGiphyItem(gif) {
  ...
  // Set direct URL first (works in Electron img tag)
  img.src = gifUrl;
  // Then try to load via fetch proxy (bypasses CSP for data URL)
...
async function init() {
  ...
  // Fix local preview checkbox: ensure it's unchecked by default
  const previewCheckbox = document.getElementById("panel-local-preview");
  if (previewCheckbox) previewCheckbox.checked = false;

  // Hide Studio section (templates, text inputs, generate button)
  const studioTabContent = document.getElementById("tab-studio");
  if (studioTabContent) studioTabContent.style.display = "none";
}
